...
```
> [!NOTE]
> **费马小定理**：如果$n$是一个素数，$a$是小于$n$的任意正整数，那么$a$的$n$次方与$a$模同余。（两个数称为是模$n$同余，如果它们除以$n$的余数相同。数$a$除以$n$的余数称为$a$取模n的余数，或简称为$a$取模$n$）
...
## 练习 1.31 求$\pi$值
$$
\prod_{n=1}^{\infin}\left(\frac{2n}{2n - 1} \cdot \frac{2n}{2n + 1}\right)
$$
